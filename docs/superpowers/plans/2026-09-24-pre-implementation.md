...
- [ ] **Passo 3: Medir o tempo do "Enviar"** (spec 7.3). Com o navegador em **Ferramentas do desenvolvedor > Rede**, filtro `pre_send.php`:
  ...
  **Resultado da execução (25/09/2026):** cada linha levou em média 0,81 s (20 linhas), o PDF de 50 linhas levou 0,99 s e nenhuma requisição passou de ~1 s. Os números, a leitura e o limite conhecido (pré-checagens numa requisição só) estão no fim de `docs/pre-manual-tests.md`.
...
> **Ajuste posterior:** o botão "Cancelar PRE" saiu de `items_toolbar.html.twig` e passou para a aba principal, ao lado de "Salvar" (`repairprotocol.form.html.twig`, com confirmação). O formulário genérico do GLPI desenha a linha de botões depois do bloco `form_fields`, então o template declara o botão dentro do bloco e um script curto o move para depois de "Salvar". A variável `can_cancel` vem de `RepairProtocol::showForm()`; o botão é um `submit` do próprio formulário porque não se aninham `<form>`.
