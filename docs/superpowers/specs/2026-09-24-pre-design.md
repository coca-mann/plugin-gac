...
| D5 | Status do ativo (`State`) nunca é criado automaticamente. A configuração mapeia papéis para `State` existentes e oferece um botão explícito "Criar novo". "Enviar" fica bloqueado com mapeamento incompleto | Não sujar a tabela de `State`; falhar antes de começar um lote |
