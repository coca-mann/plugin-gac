...
| D27 | No PDF, o número do protocolo aparece só na tabela do cabeçalho (o subtítulo abaixo do título foi retirado), e o rodapé mostra a "URL da aplicação" do GLPI (Configurar > Geral) no lugar do número do PRE, vazia se não estiver configurada | Decisão do dono: o número já está na tabela; o rodapé serve para identificar o sistema de origem |
